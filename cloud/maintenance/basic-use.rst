...
.. important:: The retention time for manually generated backups depends on the environment type. Backup retention follows the same policy as daily automated backups. The one-month retention described in the :ref:`OroCloud Architecture <cloud_architecture>` documentation applies only to production environments. For details on backup retention for different environment types, see :ref:`Environment Plans <cloud-environments>`.
