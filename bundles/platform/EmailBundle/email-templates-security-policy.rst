...
``Oro\Bundle\EmailBundle\Twig\SafeGetAttributeNodeExtension``
   Twig extension registered on the sandboxed email template environment. It installs the node visitors that replace
   every attribute access and every string coercion in the compiled template with a safe variant, and carries the
   event dispatcher those nodes report a violation through, a Twig node having no other way to reach a service.
...
On substituting a value for a denied access, see
