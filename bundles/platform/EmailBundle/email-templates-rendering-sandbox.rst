...
Set the default value via the ``@Config`` annotation on the entity class (see
:ref:`@Config <annotation-config>`):
...
Set the default value via the ``@ConfigField`` annotation on the property (see :ref:`@ConfigField email scope <annotation-config-field-email>`):
...
This is how the password reset confirmation token works. The templates Oro ships read the parameter directly,
as ``{{ confirmationToken|default('N_A') }}``; the substitution keeps a customised copy of one of them rendering
until it is updated.
