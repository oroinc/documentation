...
Client-side validation supports the same validation annotation that is used for the server-side - |Symfony validation|. Once `validation.yml` is created, all rules get translated to the fields' `data-validation` attribute, e.g.:
