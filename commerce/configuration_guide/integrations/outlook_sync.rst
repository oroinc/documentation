...
All the contacts, tasks and calendar events available for the user can be synchronized with the specified
...
Integration Settings on the Oro Side
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Synchronization Settings on the Oro Side
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. image:: /user_guide/system/img/outlook/outlook_menu_bar.png
...
^^^^^^^^^^^^^^^^^^^^^
Synchronization will start automatically, or as soon as the user has clicked the :guilabel:`StartSyncNow` button on the side panel, subject to the settings defined.
...
.. image:: /user_guide/system/img/outlook/outlook_from_oro_diag.png
...
.. image:: /user_guide/system/img/outlook/oro_from_outlook_diag.png
...
.. |alert| image:: /user_guide/system/img/outlook/alert.png
