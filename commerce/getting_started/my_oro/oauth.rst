...
To be able to add an OAuth application:

* Install `OAuth extension from Oro Marketplace <https://marketplace.oroinc.com/orocommerce/extension/oauth-2.0-client-credentials-authorization>`__.
* `Generate <https://oauth2.thephpleague.com/installation/#generating-public-and-private-keys>`__ private and public encryption keys and add them to the /var directory of the installed Oro application. Although the path to the keys is predefined, you can change it by providing your custom location in the *config.yml* file.
