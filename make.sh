...
case "$1" in 
  clean|gettext|htmlhelp|qthelp|html|dirhtml|singlehtml|pickle|json|epub|latex|text|man|texinfo|changes|linkcheck|doctest|pdf)
  #do nothing
  ;;
    *)
    usage
...
if [ "$1" = "clean" ]; then
  ...
  res=$?
  echo "Build finished. The message catalogs are in $BUILDDIR/pdf."
elif [ "$1" = "gettext" ]; then
