View and Filter Tax Rules
~~~~~~~~~~~~~~~~~~~~~~~~~

.. begin

To view all tax rules, navigate to **Taxes > Tax Rules** in the main menu.

.. image:: /user_doc/img/taxes/all_tax_rules.png
   :alt: The list of all available tax rules

.. note:: To handle a big volume of data, use the page switcher, increase *View Per Page* or use filters to narrow down the list to just the codes you need.

In the tax rules list, you will find the information about customer and product tax codes, tax jurisdiction, and tax rates that are linked to the tax rules, detailed description, and the dates when the tax rule was created and updated.

Hover over the |IcMore| **More Options** menu to the right of the item and click |IcView| to open its details page, |IcEdit| to edit, or  |IcDelete| to remove the tax rule.

.. stop

.. include:: /user_doc/img/buttons/include_images.rst
   :start-after: begin
