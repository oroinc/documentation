...
3. Make the required changes following steps 3 to 6 of the :ref:`Create an Entity <doc-entity-actions-create>` section.   
   ...
   - **Field Level ACL** --- Select this check box to define that permissions can be set on individual fields of this entity. For more information about field level ACLs, see the :ref:`Permissions for an Entity Field (Field Level ACLs) <user-guide-user-management-permissions-roles--field-level-acl>` topic.

   - **Show Restricted** --- Select this check box if you enabled **Field Level ACL** and are going to disable editing of some fields of the entity records but still want users to review the disabled fields on the interface. Fields disabled for modifying will appear dimmed on the interface. For more information about field level ACLs, see the :ref:`Permissions for an Entity Field (Field Level ACLs) <user-guide-user-management-permissions-roles--field-level-acl>` topic.

   - **Searchable** --- Defines whether record users can search for and find records of this entity via OroCRM's :ref:`search functionality <user-guide-getting-started-search>`.
