...
In OroCRM, you can configure email related settings on three :ref:`levels <configuration--guide--config-levels>`, globally, per organization and per user. Based on the level where configuration has taken place, settings can fall back to other levels. For more information on configuration levels in the application, please see the :ref:`Configuration Layers topics <configuration--guide--config-levels>`.

All email settings are located in the system, organization or user configuration settings menus, with the system settings on the global level containing the highest number of options:
