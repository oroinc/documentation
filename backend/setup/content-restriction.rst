...
2. For admins:
  ...
  The configuration above, or other configuration of you choice, will merge with the default configuration and your changes will be added to the appropriate mode.
   ...
   .. hint:: Please keep in mind that while only ``secure``, ``selective``, ``unsecure`` attributes are allowed in modes, you need to specify either ``default`` (secure) or ``lax`` (selective) when configuring further changes (e.g., changing allowed tags).
