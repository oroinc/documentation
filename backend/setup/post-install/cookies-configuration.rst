...
.. important:: Starting from platform version 5.0.4, the value of the `HTTPOnly` parameter for this cookie is always ‘false’ and cannot be changed to ‘true’ since the cookie value must be available in the JavaScript that sends AJAX requests to the server.
