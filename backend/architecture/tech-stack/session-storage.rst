...
By default, Oro application is configured to store |sessions| in files. If your application is served by multiple
...
In any distributed environment (more than one web node), Oro application needs to use shared session storage. |Redis|
is one of best-supported options with high performance. By default, |session data is locked| to avoid race conditions,
and |SncRedisBundle|, which is used in Oro to store session in Redis, is not an exception.
