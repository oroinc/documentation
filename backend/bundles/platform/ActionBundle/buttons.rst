...
Together with :ref:`Operations <bundle-docs-platform-action-bundle-operations>`, ActionBundle provides a useful way for a developer to add specific User Interface Buttons for some context matches that are common in the OroPlatform based applications.
...
You can rely on the `oro_action.button_provider.on_buttons_matched` event which is dispatched with general `@event_dispatcher` service and emits currently matched buttons collection connected to the |OnButtonsMatched| event object.
