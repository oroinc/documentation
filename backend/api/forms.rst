...
+--------------------------------+-----------------------------------------------+
| oro.api.form.type\_guesser     | Add a custom logic for "form type guessing"   |
+--------------------------------+-----------------------------------------------+
...
To switch between the general and data API forms, use the |ProcessorSharedInitializeApiFormExtension| and |ProcessorSharedRestoreDefaultFormExtension| processors.

The |ProcessorSharedBuildFormBuilder| processor builds the form for a particular entity on the fly based on the data :ref:`API configuration <web-api--configuration>` and the entity metadata.
