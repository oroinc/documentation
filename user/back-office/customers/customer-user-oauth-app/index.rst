...
3. Provide the following details on the page that opens:
   ...
   * **Enable Session Transfer** --- (available as of OroCommerce v6.1.11) Allow the application to exchange an access token issued to it for a short-lived, one-time token that creates a browser session. Enable this option only for trusted applications. See
