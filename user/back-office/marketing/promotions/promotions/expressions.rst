...
Expression language for promotions expressions is a user-friendly and business oriented extension of the |Symfony Expression Language|. On top of usual comparison and logical operators, it allows iterating through the collections of items using *(collection).all* and *(collection).any* operations. Keep in mind that out of the box, you can add any custom fields to the required entity in addition to the attributes listed below.
