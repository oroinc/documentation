...
5. Provide the UPS connection details: `OAuth Client Id` and `OAuth Client Secret` to connect. Click **Check UPS Connection** to ensure UPS API is accessible.
    ...
    .. note:: For the case when you update existing integration, you may see the additional deprecated **API user (Deprecated)**, **API Password (Deprecated)**, **API Key (Deprecated)** fields.
