...
.. |BStartfL| image:: ./img/sales_processes/Buttons/BStartfL.png
   align: middle

.. |Bplus| image:: ./img/sales_processes/Buttons/Bplus.png
   align: middle

.. |BSave| image:: ./img/sales_processes/Buttons/BSave.png
   align: middle

.. |BSubmit| image:: ./img/sales_processes/Buttons/BSubmit.png
   align: middle

.. |BCrO| image:: ./img/sales_processes/Buttons/BCrO.png
   align: middle
   
.. |BAddNote| image:: ./img/sales_processes/Buttons/BAddNote.png
   align: middle
   
.. |BSendEm| image:: ./img/sales_processes/Buttons/BSendEm.png
   align: middle
...
.. |OppCrMF| image:: ./img/sales_processes/Screenshots/OppCrMF.png
   :width: 75 %
...
.. hint:: If you cannot see the section, there may be still no B2B Channels with Leads Entity assigned to them in the 
          ...
          If so, `fill the System with B2B Channels </user_guide/sales_process.rst#fill-the-system-with-b2b-channels>`_  
...
.. hint:: You can also create a Lead from the *System -->Sales Processes* page.
          See `*Sales Processes* section  </user_guide/sales_process.rst#start-a-sales-process-from-lead>`_ for details.
...
.. caution:: `Mandatory fields </user_guide/sales_process.rst#mandatory-system-fields-for-leads>`_ **must** be 
             specified


.. hint:: You can leave "id" field empty, and the system will generate unique ids itself. Be careful, if there are 
          already some Leads in the system and you upload new ones with ids specified. If the two ids match (e.g. one from a 
          third-party application and one already in the system), the system will treat the Lead creation, as update of an 
          existing Lead.
...
- Re-define the values you need to change.
  This is similar to `creating a lead </user_guide/sales_process.rst#create-leads>`_
...
- `Import </user_guide/sales_process.rst#import-leads>`_ the edited file.
...
.. hint:: If you cannot see the section, there may be still no B2B Channels with Opportunities Entity assigned to them 
          ...
          If this is `fill the System with B2B Channels </user_guide/sales_process.rst#fill-the-system-with-b2b-channels>`_. 
...
.. note:: You can also make an Opportunity from the *System -->Sales Processes* Page, as described page as described 
          `in the *Sales Processes* section  </user_guide/sales_process.rst#start-a-sales-process-from-opportunity>`_   
...
.. list-table:: **Mandatory Opportunity Fields**
   ...
   * - **B2B Customer***
     - The field binds the Opportunity created to a specific Customer in the System. Customer entity contains all the details of one customer available in the System (e.g. shipping and banking details, data on opportunities and purchases from all Channels, etc.).
 
       Field that was optional for Leads, is mandatory for Opportunities. This is related to higher business importance of Opportunities. While almost any potentially useful acquaintance may be deemed as a Lead, Opportunities shall have high probability of turning into real sales activities, and thus it is important to keep track of the related customers information.
...
If a field refers to a number (e.g. *Probabiliy (%)*, *Budget Amount ($)*, *Close Revenue ($)*) an integer value shall 
be filled (if any).
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Opportunities import is very similar to the Leads import </user_guide/sales_process.rst#import-leads>`. You do the 
same actions from the *Sales --> Opportunities* page. 

..caution:: `Mandatory fields </user_guide/sales_process.rst#mandatory-system-fields-for-opportunities>`_ **must** be 
             specified
...
- Re-define the values you need to change.
  This is similar to `creation of an Opportunity </user_guide/sales_process.rst#create-opportunities>`_
...
It is very similar to the `Leads editing </user_guide/sales_process.rst#processing-.csv-files-to-edit-opportunities>`. 
